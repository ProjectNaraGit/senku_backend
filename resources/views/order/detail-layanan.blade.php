            ...
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-8 mb-10">
              ...
              <div class="lg:col-span-2 space-y-6">
                <div class="flex items-start">
                  <a href="{{ route('user.dashboard') }}" class="bg-white/80 hover:bg-white p-2 rounded-full shadow-lg transition flex items-center gap-2">
                ...
                <div class="w-full rounded-2xl overflow-hidden bg-[#5F6F52] relative flex items-center justify-center p-4 min-h-[260px] md:min-h-[360px]">
                  ...
                  @endphp
                  <img src="{{ $detailImage }}" alt="{{ $layanan->nama_layanan }}" class="w-full h-full object-contain" />
                </div>
              ...
              <div class="lg:col-span-1 space-y-[22px] mt-16">
                <div class="bg-white rounded-xl p-6 shadow-sm space-y-4">
                  ...
                  @if(isset($layanan->kategori) && $layanan->kategori)
                    <span class="inline-block bg-[#5F6F52] text-white text-xs font-semibold px-3 py-1 rounded">
                      {{ ucfirst($layanan->kategori) }}
                    </span>
                  @endif

                  <a href="{{ route('order.co-detail', $layanan->id) }}" class="block w-full bg-[#5F6F52] hover:bg-[#4a5841] text-white py-3 rounded-lg font-semibold transition text-center">
                    Pesan Sekarang
                  </a>

                  <form action="{{ route('cart.add', $layanan->id) }}" method="POST">
                    @csrf
                ...
                <!-- Ads -->
                <img src="{{ asset('images/iklan.png') }}" class="rounded-lg w-full" alt="Iklan Senku" />
                <img src="{{ asset('images/iklan.png') }}" class="rounded-lg w-full" alt="Iklan Senku" />
              </div>
            </div>
            ...
            <div class="space-y-6">
              ...
              <div class="bg-blue-600 rounded-xl p-6 sm:p-8 mt-10 relative overflow-hidden flex flex-col sm:flex-row items-start sm:items-center gap-6 sm:justify-between text-white max-w-7xl mx-auto">
                ...
                <div class="absolute right-0 top-0 h-full w-40 sm:w-56 pointer-events-none">
                  <img src="{{ asset('images/pattern.png') }}" class="h-full w-full object-cover opacity-90" />
                </div>
              </div>
            </div>
          </div>
