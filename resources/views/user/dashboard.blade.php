...
<head>
  ...
  <style>
    ...
    .science-splash::after {
      ...
      position: absolute;
      inset: 0;
      background-image: url('{{ asset('images/beaker.png') }}'), url('{{ asset('images/beaker_spill.png') }}');
    ...
    <div class="w-full px-5 md:px-10 lg:px-[120px]">
      <div class="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
        ...
        <div class="flex gap-4 items-center w-full justify-end md:w-auto md:ml-auto">
          ...
          <a href="{{ route('user.profile') }}#status-pesanan" class="relative font-poppins font-light text-[1.1em]" aria-label="Notifikasi pesanan">
    ...
    <section class="pattern-overlay w-full bg-[#5F6F52] px-5 md:px-10 lg:px-[120px] py-16 md:py-24">
        ...
        <div class="mb-8 flex flex-col md:flex-row gap-4 md:gap-0 md:justify-between md:items-center">
            ...
            <div class="relative w-full md:w-auto">
                ...
                >
                    <a href="{{ route('user.dashboard', ['kategori' => 'semua']) }}" class="block px-4 py-2 hover:bg-gray-100 text-[#5F6F52] font-semibold">
                        Semua Kategori
                    </a>
                    @if(isset($kategoris) && $kategoris->count() > 0)
                        @foreach($kategoris as $kat)
                            <a href="{{ route('user.dashboard', ['kategori' => $kat]) }}" class="block px-4 py-2 hover:bg-gray-100 text-[#5F6F52]">
                                {{ ucfirst($kat) }}
                            </a>
                        @endforeach
                    @endif
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 lg:gap-8">
            ...
            <a href="{{ route('layanan.detail', $item->id) }}" class="bg-white rounded-[20px] p-4 shadow-md hover:shadow-xl transition-all transform hover:scale-105 cursor-pointer flex flex-col h-full">
                @php
                    $thumbnail = $item->thumbnail_url ?? asset('images/layanan-placeholder.png');
                @endphp
                <img src="{{ $thumbnail }}" alt="{{ $item->nama_layanan }}" class="w-full h-[181px] object-cover rounded-[15px] mb-3">
                <h4 class="font-bold text-[#1B1B1B] text-[17px] mb-2 line-clamp-2 min-h-[48px]">{{ $item->nama_layanan }}</h4>
                <p class="text-[#1B1B1B] text-[13px] leading-relaxed mb-2 line-clamp-3 min-h-[60px]">{{ Str::limit($item->deskripsi_layanan, 120) }}</p>
                <div class="mt-auto min-h-[32px] flex items-end">
                    @if(isset($item->harga_layanan))
                        <p class="text-[#B56727] font-bold text-[16px]">Rp {{ number_format($item->harga_layanan, 0, ',', '.') }}</p>
                    @endif
                </div>
            </a>
            @endforeach
        ...
        <div class="flex flex-col md:flex-row items-center justify-center gap-4 md:gap-6 mt-12 text-center">
            <p class="text-[#E5E0D8] font-semibold text-2xl md:text-3xl">
                <a href="{{ route('cara_order') }}" class="hover:text-gray-300 transition">Cara Order</a>
                <span class="text-[#B56727] mx-3 md:mx-4">|</span>
                <a href="{{ route('faq') }}" class="hover:text-gray-300 transition">FAQ</a>
            </p>
        </div>
    </section>
