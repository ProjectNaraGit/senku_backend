...
<head>
  ...
  <style>
    ...
    .science-splash::after {
      content: '';
      position: absolute;
      inset: 0;
      background-image: url('<?php echo e(asset('images/beaker.png')); ?>'), url('<?php echo e(asset('images/beaker_spill.png')); ?>');
      background-repeat: no-repeat, no-repeat;
      background-size: 140px auto, 200px auto;
      background-position: left 40px top 120px, right 60px bottom 80px;
      opacity: 0.18;
      pointer-events: none;
      z-index: 0;
    }
...
<body class="bg-[#5F6F52] pattern-overlay">
  <nav class="w-full py-5 h-fit bg-[#E5E0D8] sticky top-0 z-50 border-b border-gray-700">
    <div class="w-full px-5 md:px-10 lg:px-[120px]">
      <div class="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
        ...
        <div class="flex gap-4 items-center w-full justify-end md:w-auto md:ml-auto">
          ...
          <a href="<?php echo e(route('user.profile')); ?>#status-pesanan" class="relative font-poppins font-light text-[1.1em]" aria-label="Notifikasi pesanan">
            ...
            <?php if(($notificationCount ?? 0) > 0): ?>
              <span class="absolute -top-1 -right-1 min-w-[18px] h-[18px] px-1 bg-[#B56727] text-white text-[11px] leading-[18px] rounded-full text-center font-semibold">
  ...
  <main class="science-splash">
    <section class="pattern-overlay w-full bg-[#5F6F52] px-5 md:px-10 lg:px-[120px] py-16 md:py-24">
        ...
        <div class="mb-8 flex flex-col md:flex-row gap-4 md:gap-0 md:justify-between md:items-center">
            ...
            <div class="relative w-full md:w-auto">
                <button
                    ...
                    class="bg-[#E5E0D8] hover:bg-white cursor-pointer text-[#5F6F52] px-5 md:px-6 py-3 rounded-lg font-poppins font-semibold text-[16px] flex items-center gap-2 justify-between w-full md:w-auto min-w-[200px]"
                >
                ...
                <div
                    id="kategoriDropdown"
                    class="hidden absolute right-0 mt-2 w-[200px] bg-white rounded-lg shadow-lg border z-50 max-h-[300px] overflow-y-auto"
                >
                    <a href="<?php echo e(route('user.dashboard', ['kategori' => 'semua'])); ?>" class="block px-4 py-2 hover:bg-gray-100 text-[#5F6F52] font-semibold">
                        Semua Kategori
                    </a>
                    <?php if(isset($kategoris) && $kategoris->count() > 0): ?>
                        <?php $__currentLoopData = $kategoris; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $kat): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                            <a href="<?php echo e(route('user.dashboard', ['kategori' => $kat])); ?>" class="block px-4 py-2 hover:bg-gray-100 text-[#5F6F52]">
                                <?php echo e(ucfirst($kat)); ?>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 lg:gap-8">
            ...
            <a href="<?php echo e(route('layanan.detail', $item->id)); ?>" class="bg-white rounded-[20px] p-4 shadow-md hover:shadow-xl transition-all transform hover:scale-105 cursor-pointer flex flex-col h-full">
                <?php
                    $thumbnail = $item->thumbnail_url ?? asset('images/layanan-placeholder.png');
                ?>
                <img src="<?php echo e($thumbnail); ?>" alt="<?php echo e($item->nama_layanan); ?>" class="w-full h-[181px] object-cover rounded-[15px] mb-3">
                <h4 class="font-bold text-[#1B1B1B] text-[17px] mb-2 line-clamp-2 min-h-[48px]"><?php echo e($item->nama_layanan); ?></h4>
                <p class="text-[#1B1B1B] text-[13px] leading-relaxed mb-2 line-clamp-3 min-h-[60px]"><?php echo e(Str::limit($item->deskripsi_layanan, 120)); ?></p>
                <div class="mt-auto min-h-[32px] flex items-end">
                    <?php if(isset($item->harga_layanan)): ?>
                        <p class="text-[#B56727] font-bold text-[16px]">Rp <?php echo e(number_format($item->harga_layanan, 0, ',', '.')); ?></p>
                    <?php endif; ?>
                </div>
            </a>
            <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
        </div>
        <div class="flex flex-col md:flex-row items-center justify-center gap-4 md:gap-6 mt-12 text-center">
            <p class="text-[#E5E0D8] font-semibold text-2xl md:text-3xl">
                <a href="<?php echo e(route('cara_order')); ?>" class="hover:text-gray-300 transition">Cara Order</a>
                <span class="text-[#B56727] mx-3 md:mx-4">|</span>
                <a href="<?php echo e(route('faq')); ?>" class="hover:text-gray-300 transition">FAQ</a>
            </p>
        </div>
    </section>
    <section class="pattern-overlay w-full py-16 md:py-28 bg-[#E5E0D8]">
        <div class="container mx-auto px-5 md:px-10 lg:px-[120px] flex flex-col gap-6 justify-center items-center text-center">
            <h1 class="text-[#5D6B51] font-bold text-3xl md:text-[52px]">100% <span class="text-[#B56727]">Guarantee Service</span></h1>
        </div>
    </section>
    <section class="pattern-overlay w-full py-16 md:py-28 bg-[#5D6B51]">
        <div class="container mx-auto px-5 md:px-10 lg:px-[120px] flex flex-col gap-6 justify-center items-center text-center">
            <p class="text-center font-poppins text-[#E5E0D8] text-[18px] md:text-[24px] leading-relaxed">Yuk konsultasikan dulu kebutuhanmu GRATIS,<br>mulai kejar wisudamu dari sekarang!</p>
            <a href="https://wa.me/6289699882356" target="_blank" class="py-3 px-8 bg-[#E5E0D8] rounded-lg text-[16px] md:text-[18px] font-semibold hover:bg-white transition">CTA WA</a>
        </div>
    </section>
  ...
  <script>
    ...
    document.addEventListener('click', function(event) {
      const dropdown = document.getElementById('kategoriDropdown');
      const button = event.target.closest('button[onclick="toggleKategori()"]');
      
      if (!button && !dropdown.contains(event.target)) {
        dropdown.classList.add('hidden');
        document.getElementById('kategoriIcon').classList.remove('rotate-180');
      }
    });

    // Update selected kategori text based on URL parameter
    window.addEventListener('DOMContentLoaded', function() {
      ...
      if (kategori && kategori !== 'semua') {
        selectedKategori.textContent = kategori.charAt(0).toUpperCase() + kategori.slice(1);
      } else {
        selectedKategori.textContent = 'Semua Kategori';
      }
    });
  </script>
</body>
</html><?php /**PATH C:\LARAGON\laragon\www\senku-backend\resources\views/user/dashboard.blade.php ENDPATH**/ ?>